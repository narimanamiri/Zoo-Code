...
import {
  PendingEditOperationStore,
  type PendingEditOperationInput,
} from "./PendingEditOperationStore";
    ...
    TaskProviderLike
{
  ...
  private globalStateWriteThroughTimer: ReturnType<typeof setTimeout> | null =
    null;
  ...
  private runDelegationTransition<T>(
    ...
    return runDelegationTransition(
      this.delegationTransitionLocks,
      parentTaskId,
      fn,
    );
  }
  ...
  private enqueueProviderProfileMutation<T>(
    ...
    const callerResult = this.withProviderProfileMutationTimeout(run, () => {
      controller.abort();
      this.log(
        "Provider profile mutation timed out; aborting in-flight mutation",
      );
    });
  ...
  public readonly latestAnnouncementId =
    "aug-2026-v3.76.0-dcg-providers-terminal"; // v3.76.0 destructive command guard, provider improvements, and terminal execution fix
  ...
  constructor(
    ...
    this.taskHistoryStore = new TaskHistoryStore(
  ...
  private async initializeTaskHistoryStore(): Promise<void> {
    try {
      ...
      const alreadyMigrated =
        this.context.globalState.get<boolean>(migrationKey);

      if (!alreadyMigrated) {
        const legacyHistory =
          this.context.globalState.get<HistoryItem[]>("taskHistory") ?? [];
  ...
  private handleCloudSettingsUpdate = async () => {
    this.log(
      "Ignoring cloud settings update because cloud profile synchronization is disabled",
    );
  };
  ...
  async performPreparationTasks(cline: Task) {
    ...
    if (
      ...
      cline.apiConfiguration.apiProvider === providerIdentifiers.lmstudio
    ) {
      try {
  ...
  public async evictCurrentTask(): Promise<void> {
    const current = this.getCurrentTask();
    const storedHistory = current
      ? this.taskHistoryStore.get(current.taskId)
      : undefined;
  ...
  private async markDelegatedChildInterrupted({
    ...
    if (this.taskHistoryStore.get(childTaskId)?.status === "interrupted") {
      this.log(
        `[markDelegatedChildInterrupted] Child ${childTaskId} already interrupted — skipping`,
      );
      return;
    }
    ...
    try {
      await this.runDelegationTransition(parentTaskId, async () => {
        const { historyItem: parentHistory } =
        ...
        const childHistory =
          ...
          (await this.getTaskWithId(childTaskId)).historyItem;
  ...
  public setPendingEditOperation(
    operationId: string,
    editData: PendingEditOperationInput,
  ): void {
  ...
  async resolveWebviewView(
    ...
    void this.ensureZooGatewayProfileSeeded().catch((err) => {
      this.log(
        `[ensureZooGatewayProfileSeeded] Error: ${err instanceof Error ? err.message : String(err)}`,
      );
    });
  }
  ...
  private async ensureZooGatewayProfileSeeded(): Promise<void> {
    const { getCachedZooCodeToken, getZooCodeBaseUrl } =
    ...
    const zooGatewayProfiles = allProfiles.filter(
      (p) => p.apiProvider === providerIdentifiers.zooGateway,
    );

    if (zooGatewayProfiles.length === 0) {
      this.log(
        "[ensureZooGatewayProfileSeeded] No zoo-gateway profile found, creating one",
      );
    } else {
      ...
      for (const entry of zooGatewayProfiles) {
        try {
          const fullProfile = await this.providerSettingsManager.getProfile({
            name: entry.name,
          });
          if (
            ...
            allUpToDate = false;
            this.log(
              "[ensureZooGatewayProfileSeeded] Existing zoo-gateway profile is stale, updating",
            );
            break;
          }
        } catch {
          allUpToDate = false;
          this.log(
            "[ensureZooGatewayProfileSeeded] Failed to read existing profile, will re-seed",
          );
          break;
        }
      }
      ...
      if (allUpToDate) {
        const { postZooGatewayCredentialsReady } =
          await import("../../services/zoo-gateway-credentials-sync");
        postZooGatewayCredentialsReady((message) =>
  ...
  public async createTaskWithHistoryItem(
    ...
    if (historyItem.mode) {
      ...
      const lockApiConfigAcrossModes = this.context.workspaceState.get(
    ...
    if (historyItem.apiConfigName && !skipProfileRestoreFromHistory) {
      ...
      await this.updateGlobalState("listApiConfigMeta", listApiConfig);
      const profile = listApiConfig.find(
        ({ name }) => name === historyItem.apiConfigName,
      );
  ...
  private async handleModeSwitchUnlocked(
    ...
    if (task) {
      ...
      try {
        ...
        const taskHistoryItem =
          ...
          (this.getGlobalState("taskHistory") ?? []).find(
    ...
    if (savedConfigId) {
      ...
      const currentApiConfigNameAfter = this.getGlobalState(
        "currentApiConfigName",
      );

      if (currentApiConfigNameAfter) {
        const config = listApiConfig.find(
          (c) => c.name === currentApiConfigNameAfter,
        );
  ...
  public syncClusterIntegration(options: { force?: boolean } = {}): void {
    applyClusterIntegration(this, options).catch((error) => {
      this.log(
        `AI Cluster integration failed: ${error instanceof Error ? error.message : String(error)}`,
      );
    });
  }
  ...
  private async activateProviderProfileUnlocked(
    ...
    if (!skipCurrentTaskRebuild) {
      await this.postStateToWebview();
      // Picking an AI Cluster profile from the dropdown is an activation
      // too. Only the save path used to run this, so a profile created on
      // another day never registered its MCP server or synced its skills
      // until the settings were re-saved.
      this.syncClusterIntegration();
    }
  ...
  async handleZooCodeCallback(token: string) {
    ...
    try {
      ...
      const currentApiConfigName =
        this.contextProxy.getValues().currentApiConfigName;
      ...
      const isZooGatewayActive =
        currentSettings.apiProvider === providerIdentifiers.zooGateway;
      ...
      const allProfiles = await this.providerSettingsManager.listConfig();
      const zooProfiles = allProfiles.filter(
        (p) => p.apiProvider === providerIdentifiers.zooGateway,
      );
      ...
      if (zooProfiles.length === 0) {
        ...
        await this.upsertProviderProfile(
          "Zoo Gateway",
          newConfiguration,
          isZooGatewayActive,
        );
        ...
        for (const entry of zooProfiles) {
          const isActiveProfile =
            isZooGatewayActive && entry.name === currentApiConfigName;
          const existing = await this.providerSettingsManager.getProfile({
            name: entry.name,
          });
  ...
  async exportTaskWithId(id: string) {
    ...
    const saveUri = await downloadTask(
      historyItem.ts,
      apiConversationHistory,
      defaultUri,
    );
      ...
      if (cascadeSubtasks) {
        ...
        const collectChildIds = async (taskId: string): Promise<void> => {
          try {
            ...
            console.log(
              `[deleteTaskWithId] child task ${taskId} not found, skipping`,
            );
          }
        };
  ...
  async getStateToPostToWebview(): Promise<ExtensionState> {
    ...
    return {
      ...
      cloudAuthSkipModel:
        this.context.globalState.get<boolean>("roo-auth-skip-model") ?? false,
      cloudOrganizations,
      ...
      kimiCodeIsAuthenticated: await (async () => {
        try {
          const { kimiCodeOAuthManager } =
            await import("../../integrations/kimi-code/oauth");
  ...
  > {
    ...
    return {
      ...
      destructiveCommandGuardEnabled:
        stateValues.destructiveCommandGuardEnabled ??
        DEFAULT_DESTRUCTIVE_COMMAND_GUARD_ENABLED,
      ...
      diffFuzzyThreshold:
        stateValues.diffFuzzyThreshold ?? DEFAULT_DIFF_FUZZY_THRESHOLD,
      terminalShellIntegrationTimeout:
      ...
      lockApiConfigAcrossModes: this.context.workspaceState.get(
        "lockApiConfigAcrossModes",
        false,
      ),
      ...
      autoCloseZooOpenedFilesAfterUserEdited:
        stateValues.autoCloseZooOpenedFilesAfterUserEdited,
   ...
   */
  public async broadcastTaskHistoryUpdate(
    history?: HistoryItem[],
  ): Promise<void> {
  ...
  private async cancelTaskInternal(task: Task): Promise<void> {
    ...
    try {
      ...
      if (error instanceof Error && error.message === "Task not found") {
        this.log(
          `[cancelTask] task history missing for ${task.taskId}; skipping rehydrate`,
        );
      } else {
        throw error;
      }
    ...
    if (task.parentTaskId) {
      try {
        await this.runDelegationTransition(task.parentTaskId, async () => {
          const { historyItem: parentHistory } = await this.getTaskWithId(
            task.parentTaskId!,
          );
  ...
  > {
    ...
    return {
      ...
      apiProvider:
        apiProvider && !isRetiredProvider(apiProvider)
          ? apiProvider
          : undefined,
      ...
      await this.taskHistoryStore.atomicReadAndUpdate(
        ...
        (historyItem) => {
          ...
          const childIds = Array.from(
            new Set([...(base.childIds ?? []), child.taskId]),
          );
          return {
            ...base,
    ...
    return this.runDelegationTransition(parentTaskId, async () => {
      ...
      await this.taskHistoryStore.atomicUpdatePair(
        ...
        (parent) => {
          ...
          const childIds = Array.from(
            new Set([...(parent.childIds ?? []), childTaskId]),
          );
          updatedHistory = {
            ...parent,
            status: "active" as const,
  ...
  public async abandonSubtask(childTaskId: string): Promise<boolean> {
    ...
    return this.runDelegationTransition(parentTaskId, async () => {
      ...
      this.log(
        `[abandonSubtask] Severed link between parent ${parentTaskId} and child ${childTaskId}`,
      );
      return true;
    });
  }
