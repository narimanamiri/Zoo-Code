...
describe("digestRepository", () => {
  ...
  it("takes each file's own statement of purpose rather than composing one", async () => {
    ...
    expect(server?.purpose).toBe(
      "The HTTP surface, and the only place a request is parsed.",
    );
    expect(client?.purpose).toBe(
      "Talks to the server, and retries once when it does not answer.",
    );
  });
  ...
  it("names what each file defines, and leaves the private ones out", async () => {
    ...
    expect(client?.defines).toEqual(
      expect.arrayContaining(["connect", "Session", "Options"]),
    );
  ...
  it("quotes the README whole, because it is the answer to what this is", async () => {
    ...
    expect(digest.configs.find((c) => c.path === "README.md")?.text).toContain(
      "A project that exists to be read",
    );
  });
...
describe("digestToBrief", () => {
  it("carries the real names, and says not to invent any others", async () => {
    ...
    expect(brief).toContain(
      "Do not name a file, command, framework or setting that is not listed",
    );
  });
...
describe("imports", () => {
  ...
  it("resolves a relative TypeScript import to the file it names", () => {
    ...
    const texts = new Map([
      [
        "src/tool.ts",
        'import { build } from "../services/client"' + String.fromCharCode(10),
      ],
    ]);
    expect(importEdges(files, texts)).toEqual([
      { from: "src/tool", to: "src/services/client" },
    ]);
  });
...
describe("dependencyDiagram", () => {
  ...
  it("says nothing rather than drawing a graph of one arrow", async () => {
    const digest = await digestRepository(root);
    expect(
      dependencyDiagram({ ...digest, imports: [{ from: "a", to: "b" }] }),
    ).toBeNull();
  });
});
...
describe("usage surface", () => {
  ...
  beforeAll(async () => {
    ...
    await write(
      "package.json",
      JSON.stringify({ name: "depot", scripts: { start: "node serve.js" } }),
    );
  });
  ...
  it("finds the entry point, the flags, the endpoints and the settings", async () => {
    ...
    expect(digest.usage.entryPoints.map((entry) => entry.path)).toContain(
      "serve.py",
    );
    const flags = digest.usage.commands.flatMap((file) =>
      file.flags.map((flag) => flag.name),
    );
    expect(flags).toEqual(expect.arrayContaining(["--base", "--port"]));
    expect(
      digest.usage.commands.flatMap((file) => file.commands.map((c) => c.name)),
    ).toContain("npm run start");

    const routes = digest.usage.routes.map(
      (route) => `${route.method} ${route.path}`,
    );
    expect(routes).toEqual(
      expect.arrayContaining(["GET /v1/models", "POST /v1/chat/completions"]),
    );

    const settings = Object.fromEntries(
      digest.usage.env.map((entry) => [entry.name, entry.default]),
    );
  ...
  it("puts it in the brief under the headings the cluster looks for", async () => {
    ...
    expect(brief).toMatch(
      /user-manual sections have to account for all of them/,
    );
  });
  ...
  it("leaves a bundled project's command line out of the manual", async () => {
    ...
    const bundled = await fs.mkdtemp(
      path.join(os.tmpdir(), "digest-vendored-"),
    );
    try {
      const write = async (relative: string, content: string) => {
      ...
      expect(digest.files.map((file) => file.path)).not.toContain(
        "train/gguf/tool/convert.py",
      );
      expect(digest.files.map((file) => file.path)).toContain("app/cli.py");
      expect(
        digest.vendored.some((root) => root.startsWith("train/gguf")),
      ).toBe(true);
    } finally {
  ...
  it("reads a raw-string default as the path, not as the r", async () => {
    ...
    try {
      ...
      const flags = Object.fromEntries(
        digest.usage.commands.flatMap((file) =>
          file.flags.map((flag) => [flag.name, flag.help]),
        ),
      );
