...
import {
  ...
  AI_CLUSTER_MCP_TIMEOUT_SECONDS,
  registerClusterMcpServer,
  unregisterClusterMcpServer,
...
describe("cluster MCP registration", () => {
  ...
  it("adds a streamable-http entry pointing at /mcp on the cluster's origin", async () => {
    ...
    expect((await read()).mcpServers[AI_CLUSTER_MCP_SERVER_NAME]).toEqual({
      ...
      timeout: AI_CLUSTER_MCP_TIMEOUT_SECONDS,
    });
  });

  it("gives a document build long enough to finish, unless the user chose otherwise", async () => {
    // The hub's default of 60s fails every generate_document call while the
    // file is still being written on the node.
    await fs.writeFile(settingsPath, JSON.stringify({ mcpServers: {} }));
    await registerClusterMcpServer(settingsPath, "http://10.0.0.5:18080/v1");
    expect((await read()).mcpServers[AI_CLUSTER_MCP_SERVER_NAME].timeout).toBe(
      AI_CLUSTER_MCP_TIMEOUT_SECONDS,
    );
  ...
  it("keeps other servers untouched", async () => {
    ...
    expect((await read()).mcpServers.other).toEqual({
      command: "node",
      args: ["x"],
    });
  });
  ...
  it("registers into a settings file that does not exist yet", async () => {
    expect(
      await registerClusterMcpServer(settingsPath, "http://10.0.0.5:18080"),
    ).toBe(true);
    expect((await read()).mcpServers[AI_CLUSTER_MCP_SERVER_NAME].url).toBe(
      "http://10.0.0.5:18080/mcp",
    );
  });
