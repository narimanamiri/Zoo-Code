...
describe("documentLinks", () => {
  it("finds a link mentioned in prose", () => {
    const text =
      "Built it. Download: http://cluster:18080/v1/files/9f2c1a4b7e01/Q2%20report.docx";
    expect(documentLinks(text)).toEqual([
      {
        url: "http://cluster:18080/v1/files/9f2c1a4b7e01/Q2%20report.docx",
        name: "Q2 report.docx",
      },
    ]);
  });
  ...
  it("ignores every other URL on the cluster", () => {
    expect(
      documentLinks("see http://c:18080/v1/models and http://c:18080/mcp"),
    ).toEqual([]);
  });

  it("never lets a name climb out of the directory", () => {
    const links = documentLinks(
      "http://c:18080/v1/files/abc123/..%2F..%2Fescaped.docx",
    );
...
describe("saveDocuments", () => {
  ...
  it("writes the file into the workspace and says where", async () => {
    vi.mocked(axios.get).mockResolvedValue({
      data: Buffer.from("PK pretend docx"),
    });
    const { saved, failed } = await saveDocuments(
      "Done: http://c:18080/v1/files/abc123/report.docx",
      cwd,
    );
    ...
    const written = await fs.readFile(
      path.join(cwd, "documents", "report.docx"),
    );
  ...
  it("reports a download that failed instead of pretending it worked", async () => {
    ...
    const { saved, failed } = await saveDocuments(
      "http://c:18080/v1/files/abc123/report.docx",
      cwd,
    );
  ...
  it("only fetches from the configured cluster when told which one that is", async () => {
    // Any MCP server can write a link shaped like ours into a tool result.
    vi.mocked(axios.get).mockResolvedValue({ data: Buffer.from("one") });
    const text =
      "http://c:18080/v1/files/aaa111/ours.docx and http://evil:18080/v1/files/bbb222/theirs.docx";
    const { saved, failed } = await saveDocuments(text, cwd, {
      allowedOrigin: "http://c:18080",
    });

    expect(saved.map((d) => d.name)).toEqual(["ours.docx"]);
    expect(failed[0].url).toContain("evil");
...
describe("deliveryNote", () => {
  ...
  it("says the link still works when the download did not", () => {
    expect(
      deliveryNote(
        [],
        [{ url: "http://c/v1/files/a/b.docx", error: "timeout" }],
      ),
    ).toContain("the link still works");
  });
});
...
describe("buildClusterDocument", () => {
  ...
  it("returns the bytes the cluster built", async () => {
    ...
    expect(vi.mocked(axios.post).mock.calls[0][0]).toBe(
      "http://c:18080/v1/documents/docx?download=1",
    );
  });
  ...
  it("reads a refusal out of the header, because the body is a document", async () => {
    ...
    vi.mocked(axios.post).mockResolvedValue({
      ...
      headers: {
        "x-document-refused":
          "This brief is 39 characters restating the request.",
      },
    });
    await expect(
  ...
  it("falls back to the JSON error when there is no header", async () => {
    vi.mocked(axios.post).mockResolvedValue({
      status: 400,
      data: Buffer.from(
        JSON.stringify({ error: { message: 'add "format" to the request' } }),
      ),
      headers: {},
    });
    await expect(
  ...
  it("accepts the other self-hosted profiles too", () => {
    expect(
      documentCredentials({
        ...
        lmStudioBaseUrl: "http://c:1234/v1",
      } as never)?.baseUrl,
    ).toBe("http://c:1234/v1");
    expect(
      documentCredentials({
        apiProvider: "ollama",
        ollamaBaseUrl: "http://c:11434",
      } as never)?.baseUrl,
    ).toBe("http://c:11434");
  });
