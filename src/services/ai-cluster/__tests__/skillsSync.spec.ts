...
describe("syncClusterSkills", () => {
  ...
  it("writes each skill and its inlined files", async () => {
    fetchClusterSkills.mockResolvedValue([
      summary("docforge", "aaa", [{ path: "api.md", text: "# api" }]),
    ]);
    fetchClusterSkill.mockResolvedValue(
      detail("docforge", "body", [{ path: "api.md", text: "# api" }]),
    );
    ...
    expect(await fs.readFile(path.join(dir, "SKILL.md"), "utf-8")).toContain(
      "name: docforge",
    );
  ...
  it("downloads files the list did not inline", async () => {
    fetchClusterSkills.mockResolvedValue([
      summary("docforge", "aaa", [{ path: "logo.png", bytes: 4096 }]),
    ]);
    fetchClusterSkill.mockResolvedValue(
      detail("docforge", "body", [{ path: "logo.png", bytes: 4096 }]),
    );
    fetchClusterSkillFile.mockResolvedValue(Buffer.from([1, 2, 3]));

    await syncClusterSkills(credentials);

    expect(fetchClusterSkillFile).toHaveBeenCalledWith(
      credentials,
      "docforge",
      "logo.png",
    );
    const written = await fs.readFile(
      path.join(getClusterSkillsDirectory(), "docforge", "logo.png"),
    );
  ...
  it("re-downloads when the digest moves", async () => {
    ...
    expect(
      await fs.readFile(
        path.join(getClusterSkillsDirectory(), "docforge", "SKILL.md"),
        "utf-8",
      ),
    ).toContain("second");
  });
  ...
  it("removes a skill that is gone from the cluster", async () => {
    fetchClusterSkills.mockResolvedValue([
      summary("docforge", "aaa"),
      summary("ppt-generate", "bbb"),
    ]);
    ...
    expect(
      await exists(path.join(getClusterSkillsDirectory(), "ppt-generate")),
    ).toBe(false);
  });
  ...
  it("leaves files it did not write alone", async () => {
    ...
    fetchClusterSkills.mockResolvedValue([summary("docforge", "aaa")]);
    fetchClusterSkill.mockResolvedValue(detail("docforge", "body"));
    await fs.mkdir(path.join(getClusterSkillsDirectory(), "hand-written"), {
      recursive: true,
    });
    await fs.writeFile(
      path.join(getClusterSkillsDirectory(), "hand-written", "SKILL.md"),
      "mine",
    );
  ...
  it("resyncs from scratch when the cluster address changes", async () => {
    ...
    const second = await syncClusterSkills({
      baseUrl: "http://other:18080/v1",
    });
  ...
  it("drops a bundled path that tries to climb out of the skill", async () => {
    ...
    expect(
      await exists(path.join(getClusterSkillsDirectory(), "docforge", "ok.md")),
    ).toBe(true);
  });
  ...
  it("keeps the old copy when the new one cannot be fetched whole", async () => {
    // The first version deleted the skill before fetching its files, so one
    // failed download left no skill at all until the next sync.
    fetchClusterSkills.mockResolvedValue([summary("docforge", "aaa")]);
    fetchClusterSkill.mockResolvedValue(detail("docforge", "first"));
    await syncClusterSkills(credentials);

    fetchClusterSkills.mockResolvedValue([
      summary("docforge", "bbb", [{ path: "big.bin", bytes: 9 }]),
    ]);
    fetchClusterSkill.mockResolvedValue(
      detail("docforge", "second", [{ path: "big.bin", bytes: 9 }]),
    );
    fetchClusterSkillFile.mockRejectedValue(new Error("socket hang up"));
    const result = await syncClusterSkills(credentials);

    expect(result.failed).toEqual([
      { name: "docforge", error: "socket hang up" },
    ]);
    expect(
      await fs.readFile(
        path.join(getClusterSkillsDirectory(), "docforge", "SKILL.md"),
    ...
    const leftovers = (await fs.readdir(getClusterSkillsDirectory())).filter(
      (n) => n.startsWith(".staging"),
    );
    expect(leftovers).toEqual([]);
  });

  it("removes skills of the previous cluster that the new one does not have", async () => {
    fetchClusterSkills.mockResolvedValue([
      summary("docforge", "aaa"),
      summary("only-old", "ccc"),
    ]);
    fetchClusterSkill.mockImplementation(async (_c: unknown, name: string) =>
      detail(name, "body"),
    );
    ...
    expect(second.removed).toEqual(["only-old"]);
    expect(second.updated).toEqual(["docforge"]);
    expect(
      await exists(path.join(getClusterSkillsDirectory(), "only-old")),
    ).toBe(false);
  });
