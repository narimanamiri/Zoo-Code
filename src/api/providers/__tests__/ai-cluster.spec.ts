...
type MockedOpenAI = {
	mockImplementation: (fn: (config?: { baseURL?: string; apiKey?: string }) => unknown) => void
	mockClear: () => void
}
;(OpenAI as unknown as MockedOpenAI).mockImplementation(function (config?: { baseURL?: string; apiKey?: string }) {
	return {
...
const oneChunk = () =>
	asyncStreamFrom([
		{
			choices: [{ delta: { content: "Hello", reasoning_content: "analysis…" }, index: 0 }],
			usage: null,
		},
		{
...
describe("AiClusterHandler", () => {
	...
	it("accepts a base URL with or without the /v1 suffix", () => {
		...
		expect(OpenAI).toHaveBeenCalledWith(
			expect.objectContaining({ baseURL: "http://10.0.0.5:18080/v1", apiKey: "cluster-key" }),
		)
		;(OpenAI as unknown as MockedOpenAI).mockClear()
		new AiClusterHandler({ ...options, aiClusterBaseUrl: "http://10.0.0.5:18080" })
		expect(OpenAI).toHaveBeenCalledWith(expect.objectContaining({ baseURL: "http://10.0.0.5:18080/v1" }))
	})
	...
	describe("request body", () => {
		...
		it("prefers an explicit modelMaxTokens over the model's default", async () => {
			const handler = new AiClusterHandler({ ...options, modelMaxTokens: 2048 })
			await collectStream(handler.createMessage("sys", messages))
		...
		it("sends temperature when the user chose one", async () => {
			const handler = new AiClusterHandler({ ...options, modelTemperature: 0.2 })
			await collectStream(handler.createMessage("sys", messages))
			expect(mockCreate.mock.calls[0][0].temperature).toBe(0.2)
		})

		it("passes reasoning effort as a llama.cpp template argument", async () => {
			const handler = new AiClusterHandler({ ...options, reasoningEffort: "high" })
			await collectStream(handler.createMessage("sys", messages))
	...
	describe("streaming", () => {
		it("yields text, reasoning and usage", async () => {
			...
			expect(chunks).toContainEqual({ type: "usage", inputTokens: 10, outputTokens: 4 })
		})
	...
	describe("errors", () => {
		...
		it("names the address when the node is unreachable", async () => {
			...
			await expect(collectStream(handler.createMessage("sys", messages))).rejects.toThrow(
				/unreachable at http:\/\/10\.0\.0\.5:18080\/v1/,
			)
		})

		it("explains a full queue rather than repeating the status code", async () => {
			failWith(Object.assign(new Error("server busy, try again"), { status: 503 }))
			const handler = new AiClusterHandler(options)
			await expect(collectStream(handler.createMessage("sys", messages))).rejects.toThrow(/busy/)
		})

		it("points at the /v1 suffix on a 404", async () => {
			failWith(Object.assign(new Error("Not Found"), { status: 404 }))
			const handler = new AiClusterHandler(options)
			await expect(collectStream(handler.createMessage("sys", messages))).rejects.toThrow(/\/v1/)
		})
		...
		it("reports a mid-stream failure with the same wording", async () => {
			mockCreate.mockImplementation(async () => ({
				async *[Symbol.asyncIterator]() {
					...
					throw Object.assign(new Error("server busy, try again"), { status: 503 })
				},
			}))
	...
	describe("completePrompt", () => {
		it("returns the message content", async () => {
			mockCreate.mockImplementation(async () => ({ choices: [{ message: { content: "done" } }] }))
			const handler = new AiClusterHandler(options)
